...
#
# NOTE: Taken from plantcv, and modified to remove higher-order RGB homography terms, as these unfortunately
# generated a lot of image artifacts in my use-cases.
...
def get_matrix_m(target_matrix, source_matrix):
    ...
    matrix_m    = a 3x22 Moore-Penrose inverse matrix
    matrix_b    = a 22x3 matrix of linear, square, and cubic rgb values from target_img
    ...
    # create matrix_a
    matrix_a = np.concatenate((s_r, s_g, s_b), 1)
    # create matrix_m
...
def calc_transformation_matrix(matrix_m, matrix_b):
    ...
    transformation_matrix    = a 3x3 matrix of linear, square, and cubic transformation coefficients
    ...
    # check matrix_b has 3 columns
    if np.shape(matrix_b)[1] != 3:
        fatal_error("matrix_b must have 3 columns.")
    ...
    # concatenate each product column into 3X3 transformation matrix
    transformation_matrix = np.concatenate((red, green, blue), 1)
...
def apply_transformation_matrix(source_img, target_img, transformation_matrix):
    ...
    transformation_matrix        = a 3x3 matrix of tranformation coefficients
    ...
    """
    # check transformation_matrix for 3x3
    if np.shape(transformation_matrix) != (3, 3):
        fatal_error("transformation_matrix must be a 3x3 matrix of transformation coefficients.")
    # Check for RGB input
    ...
    b = 0 + source_r * blue[0] + source_g * blue[1] + source_b * blue[2]
    g = 0 + source_r * green[0] + source_g * green[1] + source_b * green[2]
    r = 0 + source_r * red[0] + source_g * red[1] + source_b * red[2]
